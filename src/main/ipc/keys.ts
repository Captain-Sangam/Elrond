...
export function registerKeysHandlers(): void {
  ipcMain.handle('keys:get', async (_, provider: ProviderName) => {
    return getApiKey(provider)
  })

  ipcMain.handle('keys:set', async (_, provider: ProviderName, key: string) => {
    await setApiKey(provider, key)
  })

  ipcMain.handle('keys:delete', async (_, provider: ProviderName) => {
